...
export default function Home() {
  ...
  const handleClickSubmit = async () => {
    ...
    setLoading(true)
    authService.login(methods.getValues('email'), methods.getValues('password'))
      .then(async (resp) => {

        // const roleId = resp?.data?.data?.fk_ms_role === 0 ? constant.ROLE_ID_EXPERT : resp?.data?.data?.fk_ms_role;
        const user: any = {
          email: resp?.data?.results?.email,
    ...
    <>
      {loading ? <LoadingPage loading={loading} /> : (
        <Flex className='gap-10 justify-center items-center align-middle min-h-screen'>
          <Image className='hidden md:block' draggable="false" objectFit='cover' priority={true} src="/assets/images/high-school.png" alt="Logo" width={542} height={369} />
          <Box className='max-w-[424px] w-full rounded-[16px] shadow-none'>
            <Box className='m-auto rounded-[16px] p-[24px] flex flex-col gap-[40px] shadow-none'>
              <Box>
                <Text
                  as={'h1'}
                  className='font-bold text-[22px] leading-[28px]'
                >
                  Login
                </Text>
                <Text
                  as={'p'}
                  className='text-[16px] mt-2 text-[#000000]'
                >
