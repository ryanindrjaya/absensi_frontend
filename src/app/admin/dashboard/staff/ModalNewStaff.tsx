...
import { InputLabelControl, InputPasswordControl, MultiSelect, Row, SelectLabelControl, Text } from '@absensi/components/UI';
...
const ModalNewstaff = ({
    ...
    const getDetailStaff = () => {
        adminService.getTeacherById(payload.pk_teacher).then(res => {
            const data = res.data.results
            if (data) {
                methods.setValue('nik', data.nik)
                methods.setValue('fk_class', data.idClass)
                methods.setValue('name', data.name)
                methods.setValue('fk_subjects', data.subjects.map((subject: any) => subject.value))
    ...
    const onSubmit = (data: any) => {
        ...
        if (mode === 'edit') {
            ...
            adminService.updateTeacher(payload.pk_teacher, dataPayload).then(() => {
                methods.reset();
                refresh()
                setOpenModal(false)
                Swal.fire({
            ...
            adminService.postTeacher(data).then(() => {
                methods.reset();
                refresh()
                setOpenModal(false)
                Swal.fire({
