...
any) => {
  ...
  const onSubmit = (data: any) => {
    ...
    if (payloadData?.lesson_schedule_days?.length === 0) {
      alert.failed("Hari wajib di isi!");
      stopBackdrop();
      return;
    }

    if (mode === "edit") {
      payloadData.lesson_schedule_days.forEach((el: any) => {
        adminService
          .updateScheduler(payload.id, {
            ...payloadData,
            lesson_schedule_days: [el],
          })
          .then(() => {
            methods.reset();
            refresh();
